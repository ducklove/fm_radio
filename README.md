...
클래식·재즈·성악을 비롯한 장르별 음반을 검색하거나 분위기에 맞춰 계속 재생한다.
...
- **유닛 전원 문법** — 실물처럼 기기를 각각 켠다: 앰프=스피커 관문, 튜너=수신, 데크=트랜스포트(EQ·턴테이블은 자체 전원). 첫 설치는 튜너 POWER 한 번에 93.1 KBS 1FM, 이후엔 직전 채널·전원 상태 복원. 목록·미디어세션·트레이 등 리모컨 경로는 자동 점화, 예약 녹음은 DT-540 스위치드 아웃렛이 튜너·데크를 깨운다
...
- **88장·515곡 음반 카탈로그** — 클래식 63장·327곡, 재즈 5장·40곡, 성악 7장·51곡과 블루스·록·팝·일렉트로니카·펑크·레게·라틴·아프리칸·월드·디스코·스카·컨트리·앰비언트 13장·97곡
- **장르·분위기 검색** — 장르와 분위기를 각각 고르고 제목·아티스트·곡명·태그를 함께 검색해 원하는 음원만 빠르게 좁힌다
- **카페용 무한 랜덤 재생** — 검색 조건에 맞는 트랙을 셔플백으로 섞어 한 바퀴 안에서는 중복 없이 재생하고, 끝나면 다시 섞어 계속 재생한다. 연결에 실패한 파일은 건너뛴다
