...
시스템 트레이에 상주하며 라디오를 재생하는 Electron 앱. 재생 엔진은
`widget.html`(미니 플레이어)을 그대로 iframe에 띄우고, 문서화된 postMessage
API(`fmRadio:*`)를 IPC로 중계해 트레이 메뉴와 연결한다.

- **트레이 좌클릭** — 미니 플레이어(튜너 스킨) 표시/숨김
- **트레이 우클릭** — 재생/일시정지, 채널 선택, 볼륨, 로그인 시 자동 시작, 시작할 때 바로 재생
- 마지막 채널·볼륨은 `%APPDATA%\mad-for-audio-tray\settings.json`에 저장·복원
